...
async function init() {
  await Promise.all([fetchDepartments(), fetchPositions(), fetchEmployees()]);
  populateFilters();
  renderTable();
}
...
function renderTable() {
  ...
  employees.forEach((emp) => {
    ...
    row.className = emp.is_fired ? "fired-row" : "";
    row.innerHTML = `
      <td>${emp.fio}</td>
      <td>${dept} / ${pos}</td>
      <td>${emp.contact} ${emp.address ? `<br><small>${emp.address}</small>` : ""}</td>
      <td class="${emp.is_fired ? "status-fired" : "status-active"}"><span>${emp.is_fired ? "Уволен" : "Работает"}</span>
      </td>
      <td>
